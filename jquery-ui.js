...
/**
 * 创建一个jQuery 小部件
 * name：创建小部件的名称，包含命名空间。使用 . 分割
 * base：要继承的基本小部件，如果不提供默认继承之$.Widget。它必须是一个可被new的函数
 * prototype：小部件的原型对象
 * 备注：官方教程 https://learn.jquery.com/jquery-ui/widget-factory/
 */
$.widget = function( name, base, prototype ) {
	...
	// 代理的prototype，可以混合对个小部件
	var proxiedPrototype = {};

	var namespace = name.split( "." )[ 0 ];
	name = name.split( "." )[ 1 ];
	var fullName = namespace + "-" + name; // 组件全名比如：ui.dialog => ui-dialog

	// 如果未提供继承的小部件，默认继承之$.Widget
	if ( !prototype ) {
		prototype = base;
		base = $.Widget;
	}
	...
	$[ namespace ] = $[ namespace ] || {}; // 检查或创建ui命名空间
	existingConstructor = $[ namespace ][ name ]; // 保存已经存在的插件
	...
	constructor = $[ namespace ][ name ] = function( options, element ) {
		...
		/**
		 * 允许小部件没有new的关键字
		 * 这里的意思就是当前小部件继承的小部件如果没有
		 * _createWidget方法就直接调用当前这个构造函数
		 * 如果存在_createWidget方法就调用继承小部件的
		 */
	...
	/**
	 * 如果之前已经存在了对应的小部件，那么会继承之前小部件
	 * 对应的静态方法，但是三个属性会被保留不会继承
	 * version、prototype、_childConstructors
	 */
